<template>
  <div class="dashboard-panel-detail">
    <t-card title="本月销售报备情况" class="dashboard-detail-card" :bordered="false">
      <t-row :gutter="[16, 16]">
    ...
    <t-row :gutter="[16, 16]" class="row-margin">
      <t-col :xs="12" :xl="9">
        <t-card class="dashboard-detail-card" title="销售报备申请趋势" subtitle="(件)" :bordered="false">
          <template #actions>
            <t-date-range-picker
    ...
    <t-card :class="['dashboard-detail-card', 'row-margin']" title="商品满意度分布" :bordered="false">
      <template #actions>
        <t-date-range-picker
          class="card-date-picker-container"
          :default-value="LAST_7_DAYS"
